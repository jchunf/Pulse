...
echo "==> ad-hoc sign (pass 1: Sparkle helpers, leaves-first, preserving identifiers)"
...
#
# The actual cause beyond "must be ad-hoc signed" is subtler:
# `codesign --force --sign -` against a **bare Mach-O** (`Autoupdate`,
# the standalone helper, has no Info.plist) generates an
# autogenerated identifier of the form `<filename>-<hash>` — e.g.
# `Autoupdate-55554944c885cecb77e63656933767b4dcfffd2f`. Sparkle's
# internal XPC launcher checks the binary by its **identifier**, not
# by filename, and the autogenerated value doesn't match the
# expected `org.sparkle-project.Autoupdate`. The connection
# fails immediately. This was confirmed at v2.0.10 by the runtime
# `BundleSigningInspector` (PR #158).
#
# Real fix: pass `--preserve-metadata=identifier,entitlements,flags,runtime`
# on every Sparkle helper signing call so the original identifier
# (and entitlements / hardened-runtime flags) survive the re-sign.
# `entitlements` matters for the .xpc services; `runtime` matters
# for hardened-runtime settings. `flags` covers the adhoc / library-
# validation flags that Sparkle's distribution sets.
#
# Bundles (.xpc, Updater.app, Sparkle dylib, framework) read their
# identifier from Info.plist, so `--preserve-metadata=identifier` is
# a no-op for them — but adding it everywhere is harmless and keeps
# the loop uniform.
SPARKLE_FW="$FRAMEWORKS/Sparkle.framework"
SPARKLE_VERS="$SPARKLE_FW/Versions/B"
SPARKLE_PRESERVE="--preserve-metadata=identifier,entitlements,flags,runtime"
# Inner Mach-Os of nested .app and .xpc bundles. Signing the bundle
# itself (next loop) seals these too, but we sign them explicitly
# first to guarantee each inner binary has its own ad-hoc signature
...
do
    if [[ -f "$inner" ]]; then
        codesign --force $SPARKLE_PRESERVE --sign - --timestamp=none "$inner"
    fi
done
...
for nested in \
    ...
    "$SPARKLE_VERS/Updater.app" \
    "$SPARKLE_VERS/Autoupdate" \
    "$SPARKLE_VERS/Sparkle"
do
    if [[ -e "$nested" ]]; then
        codesign --force $SPARKLE_PRESERVE --sign - --timestamp=none "$nested"
    fi
done
# Framework root.
codesign --force $SPARKLE_PRESERVE --sign - --timestamp=none "$SPARKLE_FW"
