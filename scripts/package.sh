...
# Canonical location inside an .app is Contents/Resources/.
#
# Gotcha — the "top-level mirror" below is load-bearing. SwiftPM's
# Bundle.module accessor was silently regenerated by a toolchain update
# into a 2-path variant that ONLY probes
#   1) Bundle.main.bundleURL.appendingPathComponent("<name>.bundle")
#   2) a hardcoded /Users/runner/… CI build path
# — neither of which resolves in a shipped .app (resources live under
# Contents/Resources/, not at `.app/<name>.bundle`). On launch the
# accessor's closure hits `fatalError("could not load resource bundle")`
# inside Migrator.bundled()'s dispatch_once, and the process dies before
# AppDelegate.init() returns. dev-latest build 111 shipped with exactly
# this crash; v1.1.10 survived only because its cached accessor still
# had the older 3-path shape that probed Bundle.main.resourceURL first.
#
# Fix: copy each bundle to both Contents/Resources/ (the correct place
# per Apple bundle conventions, kept so future accessor variants that
# probe resourceURL still work) *and* the .app top level (where the
# current accessor actually looks). Keeps us robust to whichever
# accessor shape SPM decides to emit next.
shopt -s nullglob
